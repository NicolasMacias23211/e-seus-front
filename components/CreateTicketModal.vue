<template>
  <div v-if="modelValue"
    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[95vh] overflow-hidden animate-scale-in">
      <div class="bg-gradient-to-r from-[#021C7D] to-[#50bdeb] text-white px-6 py-5 flex items-center justify-between">
        ...
        <button @click="closeModal" class="p-2 hover:bg-white/20 rounded-lg transition-all" type="button">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="overflow-y-auto max-h-[calc(95vh-180px)] p-6">
        <div v-if="isLoadingData" class="flex flex-col items-center justify-center py-12">
        ...
        <form v-else @submit.prevent="handleSubmit">
          <div class="mb-5">
            <label for="title" class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
              <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                <path d="M2 5a2 2 0 012-2h7a2 2 0 012 2v4a2 2 0 01-2 2H9l-3 3v-3H4a2 2 0 01-2-2V5z" />
              </svg>
              Título del Ticket <span class="text-red-500">*</span>
            </label>
            <input id="title" v-model="form.ticketTitle" type="text" required maxlength="45"
          ...
          <div class="mb-5">
            <label for="description" class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
              <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
            ...
            <textarea id="description" v-model="form.ticketDescription" required maxlength="250" rows="4"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <div>
              <label for="service" class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd"
              ...
              <select id="service" v-model="form.ticketService" required
            ...
            <div>
              <label for="priority" class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd"
              ...
              <select id="priority" v-model="form.ticketPriority" required
                class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all hover:border-blue-300 bg-white">
                <option value="" disabled>Seleccione una prioridad</option>
                <option v-for="priority in priorities" :key="priority.priority_name" :value="priority.priority_name">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <div>
              <label for="client" class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                  <path
              ...
              <div class="relative">
                <input id="client" v-model="form.eUserName" @input="searchEUsers" @focus="handleFocus" @blur="hideDropdown"
            ...
            <div>
              ...
              <div class="relative">
                <input id="client" v-model="clientSearch" @input="filterClients" @focus="handleClientFocus"
                  @blur="hideClientDropdown" type="text" required autocomplete="off"
                  class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all hover:border-blue-300"
                  placeholder="Buscar cliente..." />
                <div v-if="showClientDropdown && filteredClients.length > 0"
                  class="absolute z-10 w-full mt-1 bg-white border-2 border-blue-200 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                  <div v-for="client in filteredClients" :key="client.name" @mousedown.prevent="selectClient(client)"
                    class="px-4 py-3 hover:bg-blue-50 cursor-pointer transition-all border-b border-slate-100 last:border-b-0 font-medium text-slate-700">
                    {{ client.name }}
                  </div>
                </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <div>
              <label for="program" class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                  <path
              ...
              <div class="relative">
                <input id="program" v-model="programSearch" @input="filterPrograms" @focus="showProgramDropdown = true"
                  @blur="hideProgramDropdown" type="text" required autocomplete="off" :disabled="!form.clientName"
                ...
                <div v-if="showProgramDropdown && filteredPrograms.length > 0"
                  class="absolute z-10 w-full mt-1 bg-white border-2 border-blue-200 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                  <div v-for="program in filteredPrograms" :key="program.name"
            ...
            <div>
              <label for="subprogram" class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd"
              ...
              <div class="relative">
                <input id="subprogram" v-model="subProgramSearch" @input="filterSubPrograms"
                  @focus="showSubProgramDropdown = true" @blur="hideSubProgramDropdown" type="text" required
                  autocomplete="off" :disabled="!form.programName"
                  class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all disabled:bg-slate-100 disabled:cursor-not-allowed hover:border-blue-300"
                ...
                "
                  class="absolute z-10 w-full mt-1 bg-white border-2 border-blue-200 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                  <div v-for="subprogram in filteredSubPrograms" :key="subprogram.name"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <div>
              <label for="ans" class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd"
              ...
              <select id="ans" v-model="form.ticketAns" required
                class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all hover:border-blue-300 bg-white">
            ...
            <div>
              <label for="status" class="block text-sm font-bold text-slate-700 mb-2 flex items-center gap-2">
                <svg class="w-4 h-4 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd"
              ...
              <select id="status" v-model="form.statusId" required
                class="w-full px-4 py-3 border-2 border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all hover:border-blue-300 bg-white">
                <option value="" disabled>Seleccione un estado</option>
                <option v-for="status in statusList" :key="status.id_status" :value="status.id_status">
                  {{ status.status_name }}
                </option>
              </select>
            </div>
          ...
          <div class="mb-5">
            ...
            <div
              ...
              <input id="file-upload" type="file" multiple @change="handleFileUpload" class="hidden"
      ...
      <div class="bg-slate-50 px-6 py-4 flex gap-3 justify-end border-t-2 border-slate-100">
        <button type="button" @click="closeModal" :disabled="isLoadingData"
        ...
        <button type="submit" @click="handleSubmit" :disabled="isSubmitting || isLoadingData || isUploadingFiles"
          ...
          <span v-if="!isSubmitting && !isUploadingFiles" class="flex items-center gap-2">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
            </svg>
            Crear Ticket
    ...
    <ConfirmDialog :is-visible="showConfirmClose" type="warning" title="¿Cerrar sin guardar?"
      ...
      confirm-text="Sí, Cerrar" cancel-text="Continuar Editando" @confirm="handleConfirmClose"
...
const form = reactive({
  ...
  subProgramName: "",
  statusId: "" as number | "",
  eUserName: sessionStorage.getUserInfo()?.full_name || ""
});
...
const searchEUsers = async () => {
  try {
    ...
    matchText(form.eUserName, user.full_name)
  )
...
const selectUser = (user: EUser) => {
  form.assigned_to = user.network_user;
  form.eUserName= user.full_name.toUpperCase();
